  ...
  methods: {
    ...
    resetForm(event) {
      this.childrens.forEach((item) => {
        item.errMsg = '';
        const inputComp = this.inputChildrens.find(
          (child) => child.rename === item.name
        );
        if (inputComp) {
          inputComp.errMsg = '';
        ...
        for (let key in childrens) {
          ...
          if (child.isArray) {
            if (
              ...
              newFormData[fieldName][fieldValue] = this._getValue(
                name,
                invalidFields[name]
              );
            }
    ...
    _getValue(key, value) {
      ...
      const isRuleNum = rules.find(
        (val) => val.format && this.type_filter(val.format)
      );
      const isRuleBool = rules.find(
        (val) =>
          (val.format && val.format === 'boolean') || val.format === 'bool'
      );
      // 输入值为 number
      if (isRuleNum) {
        value = isNaN(value)
