  ...
  methods: {
    ...
    setValue(value) {
      ...
      if (name) {
        ...
        if (
          !this.formRules ||
          (typeof this.formRules && JSON.stringify(this.formRules) === '{}')
        )
          return;
    ...
    async triggerCheck(value, formTrigger) {
      ...
      const inputComp = this.form.inputChildrens.find(
        (child) => child.rename === this.name
      );
